 ...
 * It's for being used to monitor an ongoing OpenTTD-server and it's games.
 * It can do updates continously or manually.
...
public class ServerHandler {
     ...
     * Be aware that if you supply no initial listeners and still tell
     * it to update once immidiately no one will receive any events.
     * An OpenTTD-server runs by default on port 3979.
     * Regardless of what you supply as an update interval it will not start
     * and continous sequence without a call to start().
     * NOTE: According to the OpenTTD-code a day is about 2 seconds if
     * the machine running it is able to do run it normally.
     * @param host              the server to monitor
     * @param localPort         the local port to use
     * @param remotePort        the remote port to use
     * @param updateInterval    update interval in milliseconds, 0 or less for manual
    ...
    /**
     * Convenience constructor for creating the server handler.
     * Be aware that if you supply no initial listeners and still tell
     * it to update once immidiately no one will receive any events.
     * Regardless of what you supply as an update interval it will not start
     * and continous sequence without a call to start().
     * Standard ports, local 2222 and remote 3979 is used.
     * NOTE: According to the OpenTTD-code a day is about 2 seconds if
     * the machine running it is able to do run it normally.
     * @param host              the server to monitor
     * @param updateInterval    update interval in milliseconds, 0 or less for manual
     ...
     * If the new update interval is set to less than or equal to 0
     * this will as a direct result terminate the automatic updating.
     * Be aware that while the method can stop the updating it will never
     * start it. A separate call to start() must be done to achieve this.
     * NOTE: According to the OpenTTD-code a day is about 2 seconds if
     * the machine running it is able to do run it normally so 2000 is the
     * lower boundary for this setting.
     ...
     * This will either start it with an initial update interval
     * set to the lower boundary of 2000 if the interval is still set to
     * -1.
     */
    public void start() {
        if (updateInterval <= 0) {
    ...
    /**
     * This method is public so that any API-user might update manually.
     * also for the automatic update process. Note that this is the same
     * method invoked internally during the automatic update process.
     * After updating the data is checked to so if anything notable has
    ...
    /**
     * This method is for checking if anything interesting has happened
    ...
    /**
     * This method will determine whether any {@link Company}'s have been removed.
     * Normally these {@link OpenTTDEvent}s will only be generated if a server
     * administrator removes {@link Company} by force.
     ...
     * @param newList           the {@link List} of {@link Company}'s from the current query
    ...
    /**
     * This method will determine whether the game is running or if it has
     * been put on hold due to no clients being connected.
     ...
     * represent the same game. The method compares all data that is
     * immutable throughout one game.
    ...
    /**
     * This method will check if a game has ended and create an {@link OpenTTDEvent}
     * if it has.
     ...
     * new {@link Company}s have come into play. Note: this method should only
     * be used when a new game has started.
     * @param lastList      the {@link List} of {@link Company}'s from the last {@link JOTLQuerier}
     * @param curList       the {@link List} of current {@link Company}'s
     * @return              an {@link OpenTTDEvent} of type COMPANY_NEW or null
     */
    ...
    /**
     * This method will determine whether a new company has climbed to the top and
     * return a {@link OpenTTD}-new-leader-event.
     * @param lastList      the {@link List} of {@link Company}'s from the last {@link JOTLQuerier}
     * @param curList       the {@link List} of current {@link Company}'s
    ...
    /**
     * This is a convenience method for firing {@link OpenTTDEvent}s
     * to any and all listeners.
    ...
    /**
     *  This method is for firing a {@link OpenTTDEvent}
     *  to any and all listeners.
     */
